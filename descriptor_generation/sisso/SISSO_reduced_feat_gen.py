...
def command_center(input_file, colin_cut=0.8, n_permutations=1000, verbose=True):
    print(f"Reading input file: {input_file}")
    df = pd.read_excel(input_file)
    metadata_cols = ["cat_substrate", "ddG"]
    ...
    filtered_iter1 = filter_features(biv_1, y, colin_cut, n_permutations)
    ...
    all_augmented = pd.concat([filtered_iter1, univ_2], axis=1)
    ...
    output_filename = input_file.replace('.xlsx', '_SISSO_reduced_Boruta.xlsx')
